...
def main():
    ...
    for ind_main, video_name in enumerate(video_lst_meta):
        ...
        video_filepath = os.path.join(seg15_folder, video_name)

        json_path = os.path.join(json_store_path, video_name.replace('.mp4', '.json'))
        #Checking if the output is already been calculated. If done, skip the iteration
        if os.path.exists(json_path):
            continue
        
        frame_paths, original_frames = frame_extraction(video_filepath)
        ...
        #Samp
        filename_store = video_name.replace('.mp4', '.json')
        ...
        try:
            ...
            vidout_filepath = os.path.join(visual_store_path, video_name)
